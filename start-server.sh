...
if [ -z "$NO_UNION" ]; then
  ...
  while true; do
    echo running /gmod-union/srcds_run -game garrysmod +maxplayers ${MAXPLAYERS:=16} +hostname \"${G_HOSTNAME:="Miniserver"}\" +gamemode ${GAMEMODE:=sandbox} "${ARGS}" +map ${MAP:=gm_construct}
  ...
  while true; do
    echo running /gmod-base/srcds_run -game garrysmod +maxplayers ${MAXPLAYERS:=16} +hostname \"${G_HOSTNAME:="Miniserver"}\" +gamemode ${GAMEMODE:=sandbox} "${ARGS}" +map ${MAP:=gm_construct}
